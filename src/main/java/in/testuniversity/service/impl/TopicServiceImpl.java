...
public class TopicServiceImpl implements TopicService {
	...
	@Override
	public List<TopicDTO> getAllTopicsByStream(Long streamId) {
		if(!streamRepository.existsById(streamId)) {
			throw new StreamNotFoundException("Stream not found with id:"+streamId);
		}
		List<Topic> topics = topicRepository.findByStreamId(streamId);
		return topics.stream().map(topicMapper::entityToDTO).collect(Collectors.toList());
	}
